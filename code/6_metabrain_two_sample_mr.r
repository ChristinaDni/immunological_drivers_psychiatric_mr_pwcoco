...
instruments<- read.table("instruments_metabrain.txt", header=T)
...
lapply(list(primary), function(x){
  ...
  x$id.exposure<- paste0( x$hgnc_symbol, ":", x$ensemblid, ":", "analysis", ":", "cis_only")
  x$exposure<- paste0(x$hgnc_symbol, ":", x$ensemblid, ":", "analysis", ":", "cis_only")
  iterations= 1:12
...
setwd("/dir/proteomewide/metabrain/inflammation/results/mr")
if(file.exists("ivw_primary_metabrain.txt") == TRUE) {
  write.table(final_ivw_res_primary,"ivw_primary_metabrain.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(final_ivw_res_primary, "ivw_primary_metabrain.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
if(file.exists("wald_primary_metabrain.txt") == TRUE) {
  write.table(wald_res_primary,"wald_primary_metabrain.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(wald_res_primary, "wald_primary_metabrain.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
}
...
#Create an indicator of cis & trans instruments included
  ...
  #Create indicators with information on the SNPs
  i$id.exposure<- paste0(i$hgnc_symbol, ":", i$ensemblid, ":", "analysis", ":", "cis_trans")
  i$exposure<- paste0(i$hgnc_symbol, ":", i$ensemblid, ":", "analysis", ":", "cis_trans")
  iterations= 1:12
...
if(file.exists("ivw_secondary_metabrain.txt") == TRUE) {
  write.table(final_ivw_res_secondary,"ivw_secondary_metabrain.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(final_ivw_res_secondary, "ivw_secondary_metabrain.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
if(file.exists("wald_secondary_metabrain.txt") == TRUE) {
  write.table(wald_res_secondary,"wald_secondary_metabrain.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(wald_res_secondary, "wald_secondary_metabrain.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
}
